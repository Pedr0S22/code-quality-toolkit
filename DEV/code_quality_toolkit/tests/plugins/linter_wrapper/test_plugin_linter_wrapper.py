...
Tests cover command construction, output parsing, severity mapping, and error handling.
"""
...
# CORREÇÃO: Importar do pacote correto 'linter_wrapper'
from toolkit.plugins.linter_wrapper.plugin import Plugin

# --- Mocks de Configuração ---

class MockLinterConfig:
    """Simula a configuração específica do [plugins.linter_wrapper] no toolkit.toml."""
    def __init__(self):
        self.enabled = True
        self.linters = ["pylint"]
...
class MockToolkitConfig:
    ...
    def __init__(self):
        # CORREÇÃO: O plugin acede a config.linter_wrapper
...
def test_a_command_construction_with_args(mock_run):
    """
    Test A: Verify that specific pylint_args are included in the subprocess command.
    """
    # 1. Arrange
    plugin = Plugin()
...
def test_c_max_issues_truncation(mock_run):
    ...
    issues = [
        {"type": "error", "line": 1, "column": 0, "message": "E1", "message-id": "E1"},
        {"type": "error", "line": 2, "column": 0, "message": "E2", "message-id": "E2"},
        {"type": "error", "line": 3, "column": 0, "message": "E3", "message-id": "E3"}
    ]
    mock_run.return_value = MagicMock(stdout=json.dumps(issues), stderr="", returncode=2)
...
def test_d_output_parsing(mock_run):
    """
    Test D: Verify correct parsing of file, line, code, and message from pylint JSON.
    """
    plugin = Plugin()
    ...
    mock_run.return_value = MagicMock(stdout=json.dumps(pylint_output), stderr="", returncode=16)
...
def test_e_severity_mapping(mock_run):
    ...
    mock_run.return_value = MagicMock(stdout=json.dumps(pylint_output), stderr="", returncode=0)
    report = plugin.analyze("code...", "file.py")
    results = report["results"]

    # Mapeamento esperado conforme _PYLINT_TYPE_TO_SEVERITY no código do colega
...
def test_g_linter_not_found(mock_run):
    """
    Test G: Verify that FileNotFoundError results in a LINTER_NOT_FOUND issue.
    """
    plugin = Plugin()
...
def test_h_timeout_handling(mock_run):
    """
    Test H: Verify that TimeoutExpired results in a LINTER_TIMEOUT issue.
    """
    plugin = Plugin()
...
def test_i_nonzero_exit_code_success(mock_run):
    """
    Test I: Verify that non-zero exit code with valid JSON does NOT cause a crash.
    """
    plugin = Plugin()
    
    pylint_output = [{"type": "error", "message": "An error", "line": 1, "message-id": "E1"}]
    
    # Exit code 2 (ou outro) é normal no pylint se houver erros
    mock_run.return_value = MagicMock(stdout=json.dumps(pylint_output), stderr="", returncode=2)
...
def test_j_invalid_json_output(mock_run):
    """
    Test J: Verify that invalid JSON output from pylint returns LINTER_OUTPUT_INVALID.
    """
    plugin = Plugin()
...
def test_k_crash_no_stdout(mock_run):
    """
    Test K: Verify that non-zero exit code WITHOUT stdout returns LINTER_ERROR.
    """
    plugin = Plugin()
