...
class RulesConfig:
    ...
    max_arguments: int = 5         # New rule: maximum number of arguments allowed in a function
...
def load_config(path: str | Path | None) -> ToolkitConfig:
    ...
    """
    config = ToolkitConfig()    # creates a configuration object populated with all the default values.
    ...
    config_path = Path(path)        # 'path' is converted to a 'pathlib.Path' object for easy file system interaction.
    ...
    with config_path.open("rb") as handle:
        data = tomllib.load(handle) # parses the contents of the TOML file into a dictionary (data).

    # == Applying Overrides from TOML Data ==

    # === Plugins sections ===
    plugins = data.get("plugins", {})   # retrieves the [plugins] section, defaulting to an empty dictionary if not found.
    enabled = plugins.get("enabled")    # checks for the 'enabled' key within that section.
    if isinstance(enabled, list) and enabled: # checks whether enabled is a non-empty list before proceeding
        config.enabled_plugins = [str(item) for item in enabled]    # overwrites the default config.enabled_plugins list
                                                                    # with the new values,
    ...
    if isinstance(rules, dict):     # ensures this section is a dictionary
        ...
        config.rules.max_line_length = int(rules.get("max_line_length", config.rules.max_line_length))
        config.rules.max_complexity = int(rules.get("max_complexity", config.rules.max_complexity))
