...
if not _LOGGER.handlers:
    ...
    if SEND_LOGS_TO_TERMINAL:
        handler = logging.StreamHandler() # StreamHandler sends log messages to the standard output stream.
    else:
        log_file_path = "toolkit.log"
        handler = logging.FileHandler(log_file_path) # Note: APPENDS the log messages to the log file.
    
    formatter = logging.Formatter("%(message)s") # ensure that only the message content of the log record is printed,
    handler.setFormatter(formatter) # applies the clean formatter to the console handler,
