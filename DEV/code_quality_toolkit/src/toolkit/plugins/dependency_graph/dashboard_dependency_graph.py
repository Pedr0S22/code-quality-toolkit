...
class Plugin:
    ...
    def generate_dashboard(
        ...
        """
        try:
            graph_data = dependency_data.get("summary", {}).get("dependency_graph", {})
    ...
    def _generate_html(self, graph_data: dict, summary: dict) -> str:
        """Gera o conteúdo HTML do dashboard (versão simplificada para demo)."""
        ...
        nodes_json = json.dumps([
            {
                "id": node,
                "name": node,
                "category": self._get_node_category(node, categories)
            }
            for node in nodes
        ])
...
<head>
    ...
    <style>
        ...
        .category-title {{ font-weight: bold; padding: 10px; background: #eee; border-radius: 5px; }}
        .module-item {{ padding: 8px; margin: 5px 0; background: #f9f9f9; border-left: 3px solid #667eea; }}
    </style>
</head>
<body>
    <div class="container">
    ...
    def _generate_categories_html(self, categories: dict) -> str:
        ...
        for cat, modules in categories.items():
            ...
            <div class="category">
                ...
                <div>
                    {''.join(f'<div class="module-item">{m}</div>' for m in sorted(modules))}
                </div>
            </div>
            """
